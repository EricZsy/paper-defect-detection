...
def warp(img):
    ...
    cv2.createTrackbar('max', 'corrected', 50, 200, nothing)
    cv2.createTrackbar('GssKrnlSz', 'corrected', 3, 10, nothing)
    ...
    while(1):
        if cv2.waitKey(30) & 0xFF== 97:
            break
        # 获取滑动值
        maxVal = cv2.getTrackbarPos('max', 'corrected')  # canny最大阈值
        kernel_size = 2 * cv2.getTrackbarPos('GssKrnlSz', 'corrected') + 1  # 高斯核大小只能是奇数
        #maxVal = 50
        #kernel_size = 7
...
test_path = './test/'
#test_path = 'E:/PycharmProjects/detection1/test_1/'
...
for w in range(1, num+1):
    ...
    while True:
        ...
        b = detect(a)
    #time1 = time.time()
    #print(str(w) + '.time: %.3f' % (time1-time0))
        if cv2.waitKey()&0XFF==98:
